...
if (isset($_POST['email'])){
    ...
    $sql = $conn->query("SELECT ID FROM student_registration WHERE EMAIL='$email'");
    if($sql->num_rows>0){
      $token=generateNewString();
 $conn->query("UPDATE student_registration SET  token='$token',tokenExpire=DATE_ADD(NOW(),INTERVAL 5 MINUTE) WHERE EMAIL='$email'");
                 ...
                 $mail ->setFrom("deepaknit028@gmail.com","CSE");
                 $mail->Subject ="Reset your password for student login DEPARTMENT OF COMPUTER SCIENCE AND ENGINEERING";
                 $mail->isHTML(true);
                 $mail->Body ="
     ...
     this request you can ignore it.</p>';<br>
                 ...
                 <a href='http://localhost/CSE/ADMIN/resetPassword.php?EMAIL=$email&token=$token'>
                 http://localhost/CSE/ADMIN/resetPassword.php?EMAIL=$email&token=$token
                 </a><br><br>
